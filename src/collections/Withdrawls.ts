      ...
      async ({ data }) => {
        ...
        const handleSendEmail = async (account: any, data: any) => {
          ...
          if (typeof account.user !== 'number') {
            ...
            try {
              await sendEmailContractWithdraw(
                userDetail.email,
                `Contract Withdrawal ${data.status} `,
        ...
        if (data.status === 'pending') {
          ...
          data.message = `Withdrawal failed. The contract has been reactivated with a balance of ${data.amount.toLocaleString(
            'en-US',
            {
              style: 'currency',
              currency: 'USD',
