...
import { getContractEquityData } from "@/lib/high-light-invest-hooks"
...
export default function ContractEquityChart({ contractId }: ContractEquityChartProps) {
    ...
    useEffect(() => {
        async function fetchEquityData() {
            try {
                const data = await getContractEquityData(contractId)
                setChartData(data)
            } catch (error) {
    ...
    if (chartData.length === 0) {
        return (
            <Card>
                <CardHeader>
                    <CardTitle>Contract Equity Performance</CardTitle>
                    <CardDescription>No equity data available for this contract.</CardDescription>
                </CardHeader>
                <CardContent>
